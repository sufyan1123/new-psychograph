...
def analyze_defense_mechanisms(conversation_text: str, participant: str) -> dict:
    ...
    in the PATIENT's messages, with a quoted example for each one found.
    """
    prompt = f"""You are a clinical psychologist analyzing a patient's chat messages with {participant}.

Conversation:
{conversation_text}

Analyze ONLY the PATIENT's messages. For each defense mechanism, count occurrences and quote one example from the text. Return ONLY valid JSON with no explanation or markdown.
...
def analyze_kpis(conversation_text: str, participant: str) -> dict:
    """
    Ask Claude to score 7 communication KPIs from 0-10 with rationale,
    plus an overall health score and a flag if the conversation is concerning.
    """
    prompt = f"""You are a clinical psychologist. Analyze the PATIENT's communication with {participant}.

Conversation:
{conversation_text}

Score each KPI 0-10 with a one-sentence rationale. Return ONLY valid JSON, no markdown.
...
def qualitative_summary(conversation_text: str, participant: str) -> dict:
    """
    Ask Claude to write brief clinical case notes about the patient's
    communication style, patterns, red flags, and therapy suggestions.
    """
    prompt = f"""You are a clinical psychologist writing brief case notes about a patient's chat with {participant}.

Conversation:
{conversation_text}

Write a clinical summary. Return ONLY valid JSON, no markdown.
